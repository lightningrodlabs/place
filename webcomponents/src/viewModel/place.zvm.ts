...
export class PlaceZvm extends ZomeViewModel {
  ...
  async getMyRenderTime(bucketIndex: number): Promise<number> {
    ...
    const rank = await this.zomeProxy.getAuthorRank({author: this.cell.address.agentId.b64, bucketIndex: bucketIndex - 1}); // Must get rank of previous bucket to determine this bucket's render time
